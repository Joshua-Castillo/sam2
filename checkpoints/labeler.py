...
class VideoLabeler:
    def __init__(self, original_video_dir, output_dir):
        ...
        # Structure: { frame_idx: [(class_id, x, y), ...] }
        self.point_labels = {} 
        ...
        if not self.frame_names:
            raise FileNotFoundError(f"No frames found in {original_video_dir}")
        ...
        # Window setup
        self.window_name = "Labeler: SAM2 & Keypoints"
    ...
    def load_csv_labels(self):
        ...
        try:
            with open(self.csv_path, 'r', newline='') as f:
                reader = csv.reader(f)
                rows = list(reader)
                if rows and rows[0][0] == "filename":
                    rows = rows[1:]

                for row in rows:
                    if len(row) < 4: continue
                    fname, cid, x, y = row[0], int(row[1]), float(row[2]), float(row[3])
                    
                    if fname in self.frame_names:
                        idx = self.frame_names.index(fname)
                        if idx not in self.point_labels: self.point_labels[idx] = []
                        existing_pts = [(c, xx, yy) for c, xx, yy in self.point_labels[idx] if c != cid]
                        existing_pts.append((cid, int(round(x)), int(round(y))))
                        self.point_labels[idx] = existing_pts
        except Exception as e:
        ...
        try:
            ...
            for idx in sorted(self.point_labels.keys()):
                ...
                for (cid, x, y) in self.point_labels[idx]:
                    rows.append([fname, cid, int(x), int(y)])
            
            rows.sort(key=lambda x: (x[0], x[1]))

            with open(self.csv_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["filename", "class", "x", "y"])
                writer.writerows(rows)
            print(f"[CSV] Saved {len(rows)} points to {self.csv_path}")
        except Exception as e:
    ...
    def apply_poly_mask(self, is_subtract):
        if len(self.poly_points) < 3: return
    ...
    def add_class_point(self, x, y):
        ...
        # Remove existing point of SAME class to avoid duplicates, keep others
    ...
    def save_mask_to_disk(self):
        ...
        if self.is_dirty: print(f"[Save] Mask Frame {self.current_idx} -> {mask_name}")
        self.is_dirty = False

    # --- TRACKING LOGIC ---
    def track_and_propagate(self):
        print(f"\n--> [Tracking] Started from Frame {self.current_idx}...")
        
        # Branch based on mode
        if self.mode == "POINT":
            print("   -> Mode is POINT: Running Optical Flow (Lucas-Kanade)...")
            self.propagate_optical_flow_points()
        else:
            # SAM, REFINE, POLY all deal with the Mask, so run SAM propagation
            print("   -> Mode is SAM/REFINE/POLY: Running SAM2 Mask Propagation...")
            self.propagate_sam_masks()

        print("--> [Tracking] Complete.")
        self.load_frame_data()
        self.show_frame()

    def propagate_sam_masks(self):
        self.commit_state_to_sam()
    ...
    def propagate_optical_flow_points(self):
        # 1. Get points in current frame
        if self.current_idx not in self.point_labels:
            print("      No keypoints in current frame to propagate.")
            return
        
        current_data = self.point_labels[self.current_idx]
        if not current_data: return

        # Prepare for calcOpticalFlowPyrLK
        p_prev = np.array([[x, y] for (_, x, y) in current_data], dtype=np.float32).reshape(-1, 1, 2)
        active_classes = [c for (c, _, _) in current_data]

        prev_img_path = os.path.join(self.video_dir, self.frame_names[self.current_idx])
        prev_bgr = cv2.imread(prev_img_path)
        prev_gray = cv2.cvtColor(prev_bgr, cv2.COLOR_BGR2GRAY)

        # Loop through FUTURE frames
        for i in range(self.current_idx + 1, len(self.frame_names)):
            curr_img_path = os.path.join(self.video_dir, self.frame_names[i])
            curr_bgr = cv2.imread(curr_img_path)
            if curr_bgr is None: break
            curr_gray = cv2.cvtColor(curr_bgr, cv2.COLOR_BGR2GRAY)

            p_next, st, err = cv2.calcOpticalFlowPyrLK(prev_gray, curr_gray, p_prev, None, **self.lk_params)
            
            st = st.reshape(-1)
            p_next_2d = p_next.reshape(-1, 2)
            
            new_frame_points = []
            valid_indices = []

            for pt_idx, is_good in enumerate(st):
                if is_good:
                    x, y = p_next_2d[pt_idx]
                    if 0 <= x < self.img_w and 0 <= y < self.img_h:
                        cls = active_classes[pt_idx]
                        new_frame_points.append((cls, int(x), int(y)))
                        valid_indices.append(pt_idx)

            if new_frame_points:
                existing = self.point_labels.get(i, [])
                tracked_classes = set([c for c,_,_ in new_frame_points])
                kept_existing = [pt for pt in existing if pt[0] not in tracked_classes]
                self.point_labels[i] = kept_existing + new_frame_points
            
            if len(valid_indices) == 0:
                print(f"      Lost all points at frame {i}.")
                break
                
            p_prev = p_next[valid_indices].reshape(-1, 1, 2)
            active_classes = [active_classes[x] for x in valid_indices]
            prev_gray = curr_gray
    ...
    def show_frame(self):
        ...
        if self.is_dirty: status += " [UNSAVED MASK]"
        else: status += " [SAVED]"
        ...
        instr = "TAB: Mode | "
        track_msg = "| t: TRACK MASK"
        
        if self.mode == "SAM": instr += "L/R: Pts"
        elif self.mode == "REFINE": instr += "L/R: Fill"
        elif self.mode == "POINT": 
            instr += "L:Add R:Del"
            track_msg = "| t: TRACK PTS"
        elif self.mode == "POLY": instr += "L:Pt R:Fill Bksp:Undo"
        
        cv2.putText(final_view, instr + track_msg, (20, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
        cv2.putText(final_view, "Nav: Arrows | Esc: Quit", (20, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
    ...
    def run(self):
        ...
        print(" 't'        : TRACK (Context aware: Mask vs Points)")
        print(" 's'        : Save Mask")
        print(" Arrows     : Navigation (Auto-Saves to Memory)")
        print(" 'z' / 'x'  : Zoom In / Out")
        print(" Mid-Drag   : Pan")
        print(" 0-9        : Select Class ID for Points")
        print(" Esc        : Quit")
        print("================\n")
        ...
        while True:
            ...
            if key_char == ord('q') or raw_key == 27:
                if self.mode == "POLY" and len(self.poly_points) > 0:
                    self.poly_points = []
                    self.show_frame()
                elif raw_key == 27:
